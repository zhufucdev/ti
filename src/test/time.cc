...
TEST(Time, Stringify) {
    ASSERT_EQ(ti::helper::to_iso_time(1318057629), "2011-10-08T07:07:09Z");
}

TEST(Time, Parse) {
    ASSERT_EQ(ti::helper::parse_iso_time("2011-10-08T07:07:09Z"), 1318057629);
}
...
(
    ...
    auto tstr = ti::helper::to_iso_time(now);
    t->bind_text(0, tstr);
    t->begin();
    delete t;
    ...
    auto row = *(t->begin());
    ASSERT_EQ(ti::helper::parse_iso_time(row.get_text(0)), now);
}